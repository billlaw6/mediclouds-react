...
class UserManage extends React.Component<UserManagePropsI, UserManageStateI> {
    ...
    batchDeactivate = (): void => {
        ...
        if (selectedRowKeys.length > 0) {
            deactivateUsers(selectedRowKeys).then((res: any) => {
                // console.log(res);
                this.fetchUserList();
            }).catch((err: any) => {
                console.error(err);
    ...
    batchActivate = (): void => {
        const { selectedRowKeys } = this.state;
        if (selectedRowKeys.length > 0) {
            activateUsers(selectedRowKeys).then((res: any) => {
                // console.log(res);
                this.fetchUserList();
            }).catch((err: any) => {
                console.error(err);
    ...
    render(): ReactElement {
        ...
        const columns: any = [
            ...
            },
            {
                ...
                onFilter: (value: boolean, record: UserI) => { return record.is_active === value; }
            },
            // {
            //     title: "操作",
            //     key: "action",
            ...
            <div className="user-manage">
                ...
                <div className="user-table">
                    ...
                    <div className="user-manage-operate">
                        <Button onClick={this.batchDeactivate}
                            disabled={selectedRowKeys.length <= 0}
                            style={{ margin: "10px" }}
                        >批量禁用</Button>
                        <Button onClick={this.batchActivate}
                            disabled={selectedRowKeys.length <= 0}
                            style={{ margin: "10px" }}
                        >批量启用</Button>
                    </div>
